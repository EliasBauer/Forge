...
Designs werden auf **claude.ai/design** (Artifacts/Canvas) erarbeitet. Die Ergebnisse werden als strukturierter **Handoff** in `docs/specs/designs/` abgelegt und dort von Claude Code implementiert.

## Workflow

1. **Design auf claude.ai/design** — Mockups, Wireframes oder Komponenten als HTML/CSS/React-Artefakte erstellen. Corporate Design (Tokens, Patterns) aus `.claude/skills/frontend/SKILL.md` als Kontext mitgeben.
2. **Handoff ablegen** — Strukturierter Ordner `docs/specs/designs/` mit folgenden Dateien:
...
4. **Visual-Feedback / Korrekturen** — Screenshots für Vergleiche in `docs/specs/designs/compare_img/` ablegen (`current.png` + `target.png`). Claude Code liest beide Bilder und fixiert Abweichungen direkt.
...
- Beim Handoff immer `.claude/skills/frontend/SKILL.md` als Kontext auf claude.ai mitgeben, damit Tokens und Patterns korrekt verwendet werden.
